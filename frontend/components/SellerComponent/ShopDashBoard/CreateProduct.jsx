...
function CreateProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!seller?._id) {
      toast.error("Unable to create product: seller information not available")
      setIsLoading(false)
      return
    }
  ...
  return isAdmin && !seller?._id ? (
    <div className="flex items-center justify-center min-h-[400px]">
      <div className="text-center">
        ...
        <p className="text-gray-500">Product creation is available for seller accounts only.</p>
      </div>
    </div>
  ) : (
